...
def gen_modules():
    ...
    for path in paths:
        ...
        for (dirpath, dirnames, filenames) in os.walk(path, True):
            dirnames[:] = [
                d for d in dirnames if d.lower() not in ('demo', 'demos', 'test', 'tests')]
            ...
            for filename in filenames:
                ...
                if extension in EXTENSIONS:
                    ...
                    modpath = shortpath[
                        :-len(extension)].replace(os.path.sep, '.')
...
if sys.version_info >= (2, 6):
    import operator
    # canonicalize the sort order
    module_names.sort(key=operator.methodcaller('lower'))
else:
    ...
    for mod in module_list:
        ...
        try:
            ...
            if mod_path and os.path.splitext(mod_path)[1] == ".py":
                ...
                merge_module_scopes(mod, root, tree, use_init_fallback=(mod_path.endswith("__init__.py")),
                                    log=False)
        except Exception:
            import traceback
