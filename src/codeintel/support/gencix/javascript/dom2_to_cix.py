...
def generateCIXFromXML(root, filepath):
    ...
    cixmodule = createCixModule(
        cixfile, os.path.basename(filepath), lang="JavaScript")
            ...
            for methodnode in interfacenode.findall('.//method'):
                ...
                for paramnode in methodnode.findall('./parameters/param'):
                    if paramnode.get("attr") == "in":
                        ...
                        addCixArgument(
                            cixelement, argname, standardizeJSType(paramnode.get("type")))
                ...
                for returnsnode in methodnode.findall('./returns'):
                    addCixReturns(
                        cixelement, standardizeJSType(returnsnode.get("type")))
...
def main():
    ...
    # Write out the tree
    f = file(os.path.join(os.path.dirname(__file__), "dom2.cix"), "w").write(
        get_cix_string(cix_dom2))
