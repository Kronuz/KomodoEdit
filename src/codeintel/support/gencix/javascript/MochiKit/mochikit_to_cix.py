...
def processSection(cix_blob, cix_module, section_type, div_tag):
    ...
    for p_tag in div_tag.findAll("p", recursive=False):
        ref_tags = p_tag.findAll(
            attrs={"class": "mochidef reference"}, recursive=False)
        if not ref_tags:
            continue
        if len(ref_tags) != 1:
        ...
        if section_type == "Errors":
            ...
            if len(name_split) == 1:
                ...
                if len(sig_split) > 1:
                    ...
                    cix_element = createCixFunction(cix_class, name)
                    setCixSignature(
                        cix_element, "%s(%s" % (name, sig_split[1]))
                else:
        ...
        if cix_element.get("ilk", None) == "function":
            ...
            if returnType:
                ...
                setCixSignature(
                    cix_element, "%s(%s => %s" % (name, sig_split[1], returnType))
            else:
                setCixSignature(cix_element, "%s(%s" % (name, sig_split[1]))

        # All done, now we need to create an alias/link from main scope
        # First, find that scope, building up a name list on the way
        scope_path = "MochiKit.%s.%s" % (
            cix_module.get("name"), cix_element.get("name"))
        createCixVariable(
            cix_blob, cix_element.get("name"), vartype=scope_path)
...
def parseWithSoup(cix_root, scopename, data):
    ...
    if api_references:
        ...
        cix_file = createCixFile(
            cix_root, "MochiKit/%s.js" % (scopename), lang="JavaScript")
...
def main(cix_filename):
    ...
    p = os.popen(
        "svn co http://svn.mochikit.com/mochikit/tags/MochiKit-%s mochikit_svn" % (MOCHIKIT_VERSION))
