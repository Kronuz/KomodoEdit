...
PyPropSet_ass_subscript(PyPropSet *self, PyObject *key, PyObject *value)
{
    ...
    if (PyBytes_Check(key)) {
        ...
        pyKeyStr = PyUnicode_AsUTF8String(key);
    } else {
        PyErr_Format(PyExc_TypeError, "expected string, %.200s found",
           key->ob_type->tp_name);
        return -1;
    ...
    else
    {
        if (PyBytes_Check(value)) {
            pyValueStr = value;
            Py_XINCREF(pyValueStr);
        } else if (PyUnicode_Check(value)) {
            pyValueStr = PyUnicode_AsUTF8String(value);
        } else {
            pyValueStr = PyObject_Bytes(value);
            if (pyValueStr == NULL) {
                return -1;
            }
        }

        valueSize = PyBytes_GET_SIZE(pyValueStr);
        valueStr = PyBytes_AS_STRING(pyValueStr);
    ...
    self->propSet->Set(PyBytes_AS_STRING(pyKeyStr), valueStr, -1, valueSize);
    Py_XDECREF(pyKeyStr);
