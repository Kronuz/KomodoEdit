                        ...
                        if len(trans_value[0]) == 0:
                            ...
                            while len(final_trans_value) > 0:
                                ...
                                if not obj.patterns.has_key(ptn1):
                                    die("Undefined pattern " + ptn1 + " in str " +
                                        trans_value[0] + ", family " + family_name, True)
                        ...
                        if cmd[0] == 'paint':
                            ...
                            if not resDefinesPath:
                                pushPopStateDirective = (
                                    "p_Tran->SetPushState(%d, %s);" % (target_state_num, self.families[new_family_name]))
                            else:
                                pushPopStateDirective = \
                                    ("%d:%d:%d" % (
                            ...
                            if not resDefinesPath:
                                eolDirective = (
                                    "p_Tran->SetEolTransition(%d, %s);" % (target_state_num, self.families[new_family_name]))
                            else:
                                eolDirective = \
                                    ("%d:%d:%d" % (
                    ...
                    if not resDefinesPath:
                        ...
                        fout.write("%d:%d:%d:%d:%d:%d:%d:%d:%d\n"
                                   % (resConstants['ASTC_CREATE_NEW_TRAN'],
                                      ...
                                      colors['upto'] == "-1" and -
                                      1 or resConstants[colors['upto']],
                                      colors['include'] == "-1" and -
                                      1 or resConstants[colors['include']],
            ...
            if node[0] == 'module':
                ...
                if synthesize_eof:
                    if common_color is not None:
                        ...
                        warn(
                            "State %s might need an explicit \\z pattern rule" % (stateName,))
